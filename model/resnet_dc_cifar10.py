...
class decom_conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, DictSize, stride=1, bias=True):
...
class ResNet_dc(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, Dictsize, stride):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, Dictsize, stride))
...
def resnet20_dc(pretrained=False, **kwargs):
    model = ResNet_dc(BasicBlock, [3, 3, 3], **kwargs)
    if pretrained:
...
def resnet32_dc(pretrained=False, **kwargs):
    model = ResNet_dc(BasicBlock, [5, 5, 5], **kwargs)
    if pretrained:
...
def resnet44_dc():
    return ResNet_dc(BasicBlock, [7, 7, 7])


def resnet56_dc(pretrained=False, **kwargs):
    model = ResNet_dc(BasicBlock, [9, 9, 9], **kwargs)
    if pretrained:
...
def resnet110_dc():
    return ResNet_dc(BasicBlock, [18, 18, 18])


def resnet1202_dc():
    return ResNet_dc(BasicBlock, [200, 200, 200])
